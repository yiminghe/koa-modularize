  ...
  content = contentComment.content.replace(requireRegExp, function (match, quote, dep) {
    ...
    if (startsWithPackageName(dep)) {
      ...
      if (typeof packagePath === 'string') {
        ...
        return leading + quote + prefix + packagePath + quote + ')';
      } else {
        return packagePath;
      }
      ...
      if (typeof fullPath === 'string') {
        ...
        fullPath = leading + quote + prefix + fullPath + quote + ')';
      }
      return fullPath;
    } else {
      return match;
...
function completeCssImport(file, content, option) {
  ...
  content = contentComment.content.replace(importRegExp, function (match, quote, dep) {
    if (startsWithPackageName(dep)) {
      var fullPath = getFullPathFromTopModuleName(file, dep, option);
      if (fullPath.charAt(0) !== '/') {
        prefix = '';
      }
      return '@import ' + quote + prefix + fullPath + quote;
    } else {
      return match;
    }
  });
