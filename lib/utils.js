...
if (fs.existsSync(path.join(cwd, 'package.json'))) {
  currentPackage = require(path.join(cwd, 'package.json'));
}
  ...
  do {
    ...
    if (fs.existsSync(packageFile)) {
      ...
      if (!suffix) {
        var main;
        // debug src,lib
        if (packageInfo.name === currentPackage.name) {
          main = 'index';
        } else {
...
function getFullPathFromTopModuleName(file, name, option) {
  ...
  if (!packagePath) {
    ...
    if (packageInfo.name !== packageName && !hasDep(packageInfo, packageName)) {
