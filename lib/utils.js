  ...
  return matchRequire.replaceAll(content, function (match, quote, dep) {
    if (startsWithPackageName(dep)) {
      ...
      if (depInfo.packageName === pkg.name) {
        return getRequireCall(path.join('/', depInfo.suffix || 'index.js'), quote);
      }
      var fullPath = resolve.sync(dep, {
        filename: file,
        modules: modules
      });
      //console.log(dep + ' from ' + file + ' : ' + fullPath);
      return getRequireCall(path.join('/', path.relative(cwd, fullPath)), quote);
    } else {
      return match;
    }
  });
